...
function canControl(){ const me=getMe(); return !!(state?.started && !state?.ended && me && me.alive && !me.upgradesOpen); }
...
if (isTouchDevice) {
  ...
  canvas.addEventListener('pointerdown', e => { if(!canControl() || e.clientX < W * 0.45) return; touchState.attackId = e.pointerId; setAimFromClientPoint(e.clientX, e.clientY); firing = true; });
  canvas.addEventListener('pointermove', e => { if (touchState.attackId !== e.pointerId || !canControl()) return; setAimFromClientPoint(e.clientX, e.clientY); firing = true; });
  ...
  touchDashBtn.addEventListener('click', () => { if(canControl()) socket.emit('dash'); });
  touchSkillBtn.addEventListener('click', () => { if(canControl()) socket.emit('skill'); });
}
function sendInput(){ if(!socket.connected) return; if(!canControl()){ socket.emit('input',{ input:{up:false,down:false,left:false,right:false,firing:false}, aimX:mouse.x, aimY:mouse.y }); return; } socket.emit('input',{ input:{...keys,firing}, aimX:mouse.x, aimY:mouse.y }); }
...
function drawPlayer(me,p){ const pos=worldToScreen(me,p.x,p.y), bob=Math.sin((Date.now()/130)+(p.x+p.y)*0.01)*1.8, c=clsColor(p.cls); ctx.save(); ctx.translate(pos.x,pos.y+bob); ctx.fillStyle='rgba(0,0,0,.22)'; ctx.beginPath(); ctx.ellipse(0,16,14,6,0,0,Math.PI*2); ctx.fill();
  if(p.cls==='mage'){ ctx.fillStyle='#6d7dff'; ctx.beginPath(); ctx.moveTo(-12,10); ctx.lineTo(0,-12); ctx.lineTo(12,10); ctx.closePath(); ctx.fill(); ctx.fillStyle='#9ec7ff'; ctx.fillRect(-10,-6,20,14); ctx.fillStyle='#edd2b0'; ctx.fillRect(-7,-18,14,12); ctx.fillStyle='#4f58a6'; ctx.fillRect(-11,-22,22,6); ctx.strokeStyle='#b8ecff'; ctx.lineWidth=3; ctx.beginPath(); ctx.moveTo(10,-6); ctx.lineTo(18,-16); ctx.stroke(); ctx.beginPath(); ctx.arc(20,-18,4,0,Math.PI*2); ctx.stroke(); }
  else if(p.cls==='rogue'){ ctx.fillStyle='#7b5a22'; ctx.fillRect(-10,-8,20,17); ctx.fillStyle='#ffd76d'; ctx.fillRect(-8,-18,16,12); ctx.fillStyle='#2f2614'; ctx.beginPath(); ctx.moveTo(-9,-18); ctx.lineTo(0,-26); ctx.lineTo(9,-18); ctx.closePath(); ctx.fill(); ctx.strokeStyle='#e9f4ff'; ctx.lineWidth=2.5; ctx.beginPath(); ctx.moveTo(7,-2); ctx.lineTo(18,-6); ctx.stroke(); }
  else { ctx.strokeStyle='#dfe9f5'; ctx.lineWidth=4; ctx.beginPath(); ctx.moveTo(6,-2); ctx.lineTo(18,-12); ctx.stroke(); ctx.fillStyle='#ffce7c'; ctx.fillRect(5,-2,4,8); ctx.fillStyle='#78e7ac'; ctx.fillRect(-10,-8,20,18); ctx.fillStyle='#c8f5d8'; ctx.fillRect(-8,-18,16,12); }
  ctx.fillStyle='#162033'; ctx.fillRect(-4,-14,3,3); ctx.fillRect(1,-14,3,3); if(!p.alive){ ctx.fillStyle='rgba(0,0,0,.45)'; ctx.fillRect(-12,-22,24,38); } ctx.restore(); }
function drawEnemy(me,e){ const pos=worldToScreen(me,e.x,e.y); ctx.save(); ctx.translate(pos.x,pos.y+Math.sin(Date.now()/170+e.x*0.01)*1.5); ctx.fillStyle=e.color; if(e.kind==='normal'){ ctx.beginPath(); ctx.arc(0,0,e.r,0,Math.PI*2); ctx.fill(); } else if(e.kind==='fast'){ ctx.fillRect(-e.r,-e.r,e.r*2,e.r*2); } else if(e.kind==='tank'){ ctx.fillRect(-e.r,-e.r*.8,e.r*2,e.r*1.6); ctx.strokeStyle='#ffffff55'; ctx.lineWidth=3; ctx.strokeRect(-e.r,-e.r*.8,e.r*2,e.r*1.6); } else if(e.kind==='elite'){ ctx.beginPath(); ctx.moveTo(0,-e.r); ctx.lineTo(e.r,0); ctx.lineTo(0,e.r); ctx.lineTo(-e.r,0); ctx.closePath(); ctx.fill(); } else { ctx.fillRect(-e.r,-e.r*.85,e.r*2,e.r*1.7); ctx.strokeStyle='#fff0b8'; ctx.lineWidth=4; ctx.strokeRect(-e.r,-e.r*.85,e.r*2,e.r*1.7); } ctx.restore(); const w=Math.max(30,e.r*2.1); ctx.fillStyle='rgba(0,0,0,.35)'; ctx.fillRect(pos.x-w/2,pos.y-e.r-14,w,5); ctx.fillStyle='#7ef0b2'; ctx.fillRect(pos.x-w/2,pos.y-e.r-14,w*(e.hp/e.maxHp),5); }
...
function render(){ requestAnimationFrame(render); ctx.clearRect(0,0,W,H); if(!state || !state.players?.length) return; const me=state.players.find(p=>p.id===myId)||state.players[0]; if(!me) return; const bg=ctx.createLinearGradient(0,0,0,H); bg.addColorStop(0,'#22405f'); bg.addColorStop(.55,'#29462f'); bg.addColorStop(1,'#1b2a19'); ctx.fillStyle=bg; ctx.fillRect(0,0,W,H); drawGrid(me); drawWorldDecor(me); for(const g of state.gems||[]) drawGem(me,g); for(const b of state.bullets||[]) drawBullet(me,b,false); for(const b of state.enemyBullets||[]) drawBullet(me,b,true); for(const e of state.enemies||[]) drawEnemy(me,e); for(const p of state.players||[]) drawPlayer(me,p); }
render();
function updateUI(){ if(!state) return; const me=state.players.find(p=>p.id===myId); const other=state.players.find(p=>p.id!==myId); roomCodeEl.textContent=state.code||'----'; roomInfo.textContent=`房间 ${state.code||'----'}`; roomDebug.textContent=`房间玩家数：${state.players.length} · 已开始：${state.started?'是':'否'}${state.started?'':' · 等待准备中'}`; waveText.textContent=`第 ${state.wave} 波`; playersList.innerHTML=''; for(const p of state.players){ const div=document.createElement('div'); div.className='playerItem'; div.innerHTML=`<b style="color:${clsColor(p.cls)}">${p.name}</b> · ${clsName(p.cls)} · ${p.ready?'已准备':'未准备'}`; playersList.appendChild(div);} meInfo.textContent=me?`我：${me.name} Lv.${me.level} HP ${me.hp}/${me.maxHp} 击杀 ${me.kills}`:'我：-'; teamInfo.textContent=other?`队友：${other.name} Lv.${other.level} HP ${other.hp}/${other.maxHp} 击杀 ${other.kills}`:'队友：等待加入'; if(me){ heroBadge.textContent=clsName(me.cls); heroBadge.style.color=clsColor(me.cls); levelText.textContent=`Lv.${me.level}`; hpText.textContent=`${me.hp}/${me.maxHp}`; xpText.textContent=`${me.xp}/${me.xpNeed}`; hpFill.style.width=`${Math.max(0,Math.min(100,me.hp/me.maxHp*100))}%`; xpFill.style.width=`${Math.max(0,Math.min(100,me.xp/me.xpNeed*100))}%`; }
  chatLog.innerHTML=(state.chat||[]).map(c=>`<div><b>${c.name}：</b>${c.text}</div>`).join(''); chatLog.scrollTop=chatLog.scrollHeight; leaderboardList.innerHTML=(state.leaderboard||[]).map((e,i)=>`<div class="leaderItem"><span>${i+1}. ${e.name} (${clsName(e.cls)})</span><span>${e.cleared?'通关':'第'+e.wave+'波'} · ${e.kills}杀</span></div>`).join('');
  if(me?.upgradesOpen){ upgrades.classList.remove('hidden'); upgradeList.innerHTML=me.upgradeOptions.map((u,idx)=>`<button class="upgradeBtn" onclick="pickUpgrade('${u.key}')">${idx+1}. ${u.title}</button>`).join(''); } else upgrades.classList.add('hidden');
  if(state.ended&&state.overSummary){ summary.classList.remove('hidden'); summaryTitle.textContent=state.overSummary.cleared?'通关成功':'对局失败'; summaryBody.innerHTML=state.overSummary.players.map(p=>`<div>${p.name} · ${clsName(p.cls)} · Lv.${p.level} · ${p.kills} 击杀</div>`).join(''); } else summary.classList.add('hidden');
  if(!state.started){ resetInputs(); releaseTouchMove(); }
...
window.addEventListener('mousemove',e=>{ if(isTouchDevice || !canControl()) return; mouse.x=e.clientX-W/2; mouse.y=e.clientY-H/2; const len=Math.hypot(mouse.x,mouse.y)||1; mouse.x/=len; mouse.y/=len; }); window.addEventListener('mousedown',()=>{ if(isTouchDevice || !canControl()) return; firing=true;}); window.addEventListener('mouseup',()=>{ if(isTouchDevice) return; firing=false;});
window.addEventListener('keydown',e=>{ const k=e.key.toLowerCase(); if(k==='w'||e.key==='ArrowUp') keys.up=canControl(); if(k==='s'||e.key==='ArrowDown') keys.down=canControl(); if(k==='a'||e.key==='ArrowLeft') keys.left=canControl(); if(k==='d'||e.key==='ArrowRight') keys.right=canControl(); if(e.code==='Space' && canControl()) socket.emit('dash'); if(k==='q' && canControl()) socket.emit('skill'); if(k==='1'||k==='2'||k==='3'){ const me=state?.players?.find(p=>p.id===myId); if(me?.upgradesOpen){ const option=me.upgradeOptions[Number(k)-1]; if(option) socket.emit('pickUpgrade',{key:option.key}); } } });
